...
When opening the application for the first time, users are greeted with a welcome screen
...
The purpose of this screen is to act as the main entry point into the application,
...
The home screen provides access to the core features of the application through an interactive map
...
A floating button allows logged-in users to create a new path, leading them to the
creation flow.
...
Guest users can only perform path searches and browse suggested bike paths, but cannot
...
When a guest user attempts to interact with any restricted feature, the application
...
After submitting an origin and a destination, the application displays the search results showing
...
and displays it on the map. The selection is visually indicated by highlighting the corresponding
path card in the list, which appears with a blue, making it clearly distinguishable from
the other suggestions. Tapping the same path again deselects it, while tapping a different path
...
During an active trip, the application switches to a dedicated navigation view showing the user's
...
path. In all cases, the application stops the trip and notifies the user that the session has ended.

If the user is logged-in, the completed trip is then saved to their activity history. If the user is
a guest, no information is saved.

The \textbf{report button} allows logged-in users to submit manual reports at any moment during
the trip. This feature is disabled in guest mode. In guest mode, all trip-related controls
appear greyed out, no reports can be submitted, and no trip data is stored.
...
At the end of a trip, the application displays a pop-up informing the user that the navigation 
...
During the path creation, the application switches to a dedicated creation view showing the user's
...
After the path is successfully stored, the application displays a confirmation message 
...
This actions are available only to logged-in users.
...
When the user selects the \textbf{Start} button inside a path entry, the application redirects
...
to customize their application preferences and manage their privacy settings. The screen
...
The \textbf{Contact Us} option redirects the user to the application's store screen (Google
...
\subsection{Personal Informations}
...
the updated information. The application validates the input and, if the update is successful,
...
\begin{figure}[H]
    ...
    \caption{Personal Informations Mockup}
    \label{fig:personal_informations_mockup}
\end{figure}

\subsection{Error Pop-ups}
\label{sec:error_popups}%

The application includes a global error-handling mechanism to ensure consistent feedback
