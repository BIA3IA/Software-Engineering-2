\newcommand{\phoneimage}[2][]{%
	\begin{tikzpicture}[baseline=(img.base)]
		...
		($(topcenter)+(0cm,-0.23cm)$) circle (0.10cm);
...
All mockups presented in this chapter are \textbf{conceptual prototypes} designed to illustrate the expected
...
This chapter provides a \textbf{high-level overview} of the system's user experience. Its purpose is not to
...
area (Profile, Edit Profile, Settings). The bottom navigation bar connects Home, Trip History, Path Library,
...
Navigation begins at the \textbf{Welcome Screen}, where users may authenticate or proceed as guests. In either case,
they are redirected to the \textbf{Home Screen}, which functions as the central hub for all subsequent interactions.
...
transitions to a dedicated full-screen \textbf{Trip Navigation View}. Once the trip is completed or interrupted,
the interface automatically returns to the map-based \textbf{Home Screen}.
Trip exploration is accessible at any moment through the \textbf{Trip History Screen}, reachable via the navigation
bar. Here, users can review previous activities and inspect detailed trip information. A similar
list-and-detail pattern governs the \textbf{Path Library Screen}, which enables users to manage their custom paths,
start navigation from a created route, or adjust visibility and deletion settings. Account-related
functionality is grouped within the \textbf{Profile Screen} and its associated views (\textbf{Edit Profile} and \textbf{Settings}).
...
The \textbf{Welcome Screen} introduces the user to the application through a minimal layout featuring the app
...
Input validation and authentication errors are handled through inline messages and global pop-ups,
without altering the navigation flow.

\subsection*{Signup Screen}
\label{sec:signup_screen}%

The \textbf{Signup Screen} enables new users to create an account by entering a username, email address,
password, and password confirmation. Input validation errors are handled through inline messages.
The interface intentionally keeps the registration process
...
The \textbf{Home Screen} serves as the central hub of the BBP application, providing access to map exploration,
...
\textbf{Home}, \textbf{Trip History}, \textbf{Path Library}, and \textbf{Profile}.
...
triggers an authentication pop-up inviting the user to log in. This design clearly communicates
...
one primary action, \textbf{Log In}, which redirects the user to the Login Flow.
...
controls. The pop-up may be closed by tapping
...
After submitting a path search from the \textbf{Home Screen}, the application displays an
\textbf{Available Paths} panel anchored to the top of the map. The panel lists all suggested
routes that match the selected criteria. Each entry shows the path name, a short description
when available, the estimated distance and the current condition of the path
(e.g., Optimal, Maintenance), accompanied by the number of aggregated reports, if any.
...
the system displays a modal prompt at the top of the map asking: \textbf{"Are you biking?"}. This prompt
...
polyline. This provides an immediate visual
...
Guest users may start navigation from a selected path, but their activity is not stored.
...
During an active trip, the interface transitions to a dedicated full-screen \textbf{Navigation View}.
...
A \textbf{Complete Trip} button is anchored at the bottom of the screen, allowing the user
...
A trip ends when the user presses the \textbf{Complete Trip} button, upon reaching the destination, or when
...
For logged-in users, the pop-up shows the message \textbf{"Great Ride!"} along with a \textbf{View Stats}
...
For guest users, the pop-up displays \textbf{"Trip Complete"} together with a reminder that trip statistics
...
After the report is submitted, the system displays a confirmation pop-up titled \textbf{"Report
...
appears at the top of the screen with the message \textbf{"Obstacle Detected"}, indicating that
...
\textbf{"Path Created!"}. The dialog informs the user that the route has been successfully stored and
is now available for use.

The pop-up includes a single action button, \textbf{Go to Path Library}, which redirects the user to the
...
The \textbf{Trip History Screen} is accessible through the second icon in the bottom navigation bar and
...
the corner of the map. Beneath the map, a performance panel displays additional metrics such as distance,
...
The \textbf{Path Library Screen} displays all custom bike paths created by the logged-in user and is accessible
...
\label{sec:profile_and_settings_for_logged_in_users}%
\vspace{-1em}
...
riding time, average and maximum speed, and other performance
...
fields for the Username and Email Address, followed by a dedicated section for
changing the password.

To update the password, the user must enter their Current Password, then specify a
New Password, and confirm it in the Confirm Password field. This ensures the new
...
is automatically applied during path creation but can still be overridden.
