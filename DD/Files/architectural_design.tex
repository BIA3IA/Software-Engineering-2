...
\item \textbf{Input validation and error handling}.  
      Validation occurs at the system boundary (API Gateway), ensuring malformed requests
      ...
      consistent messages.  
      Domain-specific errors (e.g., \texttt{NO\_ROUTE}, \texttt{NOT\_FOUND}) are explicitly
      mapped and surfaced through the Gateway.

\item \textbf{Caching and rate limiting}.  
      Gateway-level caching may be applied on read-heavy endpoints (e.g., path metadata or
      weather snapshots) to reduce backend load.  
      Basic rate limiting is configured to protect the system against abusive clients while
      preserving normal usage.
