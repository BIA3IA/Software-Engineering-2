...
\begin{itemize}
	...
	\item \textbf{WeatherAPI}: endpoints for trip weather enrichment and retrieval.
\end{itemize}
...
data retrieved from the \textbf{WeatherManager}.
...
meteorological information. It aggregates weather data by sampling coordinates along the trip route,
including path segment polylines and origin/destination when available. The resulting weather snapshot
is returned to the \textbf{TripManager}, which stores it through the \textbf{QueryManager}.
...
\textbf{WeatherManager} on demand.
...
\begin{itemize}
	\item \textbf{enrichTrip(tripId, userId)}: samples coordinates along the trip, fetches and aggregates weather data, stores it, and returns the snapshot.
	\item \textbf{getTripWeather(tripId, userId)}: retrieves stored weather data for an existing trip.
\end{itemize}
...
\paragraph{\textbf{POST} \texttt{/weather/trips/:tripId/enrich}}
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, data \}}
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_TRIP\_ID}, \texttt{NO\_COORDINATES}
		      \item \textbf{404 Not Found}: \texttt{TRIP\_NOT\_FOUND}
	      \end{itemize}
\end{itemize}

\paragraph{\textbf{GET} \texttt{/weather/trips/:tripId}}
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, data \}}
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_TRIP\_ID}
		      \item \textbf{404 Not Found}: \texttt{TRIP\_NOT\_FOUND}, \texttt{WEATHER\_NOT\_FOUND}
	      \end{itemize}
\end{itemize}
