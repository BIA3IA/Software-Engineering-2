...
\begin{itemize}
	...
	\item \textbf{CacheingService}: provides in-memory caching capabilities
	      to improve performance for frequently accessed data, such as
	      user statistics.
\end{itemize}
...
\subsubsection{CacheingService}
The \textbf{CacheingService} interacts with the StatsManager to cache user statistics,
avoiding the need to recalculate them each time they are requested. This mechanism
avoids recomputing statistics when a user wants to access them, 
if the number of trips has not changed, improving performance
...
\begin{itemize}
	...
	\item \textbf{Application Tier:}
		  ...
		  The cacheing service is also deployed as a separate container, accessible only by the backend replicas.
...
which are then stored in the database through the \textbf{QueryManager} and 
cached via the \textbf{CacheingService}.
...
If valid statistics already exist, the StatsManager tries to load them
from the cache. If the cached data is not available,
they are loaded from the database.
Either way, they are returned to the mobile app with a \textbf{200 OK} response.
...
statistics, updates the cache through the \textbf{CacheingService},
...
\subsection*{CacheingService}
The \textbf{CacheingService} provides in-memory caching for frequently accessed data,
reducing database load and improving response times. It is used by the \textbf{StatsManager}.

\begin{itemize}
	\item \textbf{insertCachedStats(stats, userId)}: inserts cached statistics for a user.
	\item \textbf{getCachedStats(userId)}: retrieves cached statistics for a user, if available.
\end{itemize}
