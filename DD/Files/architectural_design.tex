...
\begin{itemize}
	\item \textbf{createTrip(tripPayload, userId)}: stores a completed trip and triggers weather/statistics enrichment when required.
...
\begin{itemize}
	\item \textbf{getReportsByPath(pathId)}: retrieves reports associated with a selected path.
	\item \textbf{createReport(reportPayload, userId)}: stores a new report, either manual or automatic.
	\item \textbf{confirmReport(reportId, decisionPayload, userId)}: creates a confirmation or rejection entry for an existing report.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{getTripStats(tripId)}: returns cached trip statistics or computes them on demand.
	\item \textbf{computeTripStats(tripPayload)}: derives metrics (distance, duration, avg speed) from trip samples.
\end{itemize}
...
\begin{itemize}
	\item \textbf{geocodeLocation(locationString)}: returns coordinates for a textual location.
\end{itemize}
...
\begin{itemize}
	\item \textbf{snapCoordinates(coordinates)}: snaps a list of coordinates to the road network.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{createReport(userId, pathId, position, payload)}: inserts a new obstacle report.
	\item \textbf{getReportsByPathId(pathId)}: retrieves reports for a path.
	\item \textbf{createReportConfirmation(userId, reportId, decision)}: inserts a confirmation or rejection.
	\item \textbf{getOverallStatsByUserId(userId)}: retrieves cached overall statistics.
	\item \textbf{upsertOverallStats(userId, statsPayload)}: creates or updates overall statistics.
	\item \textbf{getTripStatsByTripId(tripId)}: retrieves cached per-trip statistics.
	\item \textbf{upsertTripStats(tripId, statsPayload)}: creates or updates per-trip statistics.
...
\begin{itemize}
	...
	\item \textbf{Responses}:
	      \begin{itemize}
		      ...
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_PATH\_SEGMENTS}, \texttt{MISSING\_VISIBILITY}, \texttt{INVALID\_SEGMENT}, \texttt{MISSING\_CREATION\_MODE}, \texttt{INVALID\_CREATION\_MODE}, \texttt{NOT\_IMPLEMENTED}
		      \item \textbf{401 Unauthorized}: \texttt{USER\_NOT\_AUTHENTICATED}
		      \item \textbf{404 Not Found}: \texttt{PATH\_NOT\_FOUND\_AFTER\_CREATION}
		      \item \textbf{409 Conflict}: \texttt{PATH\_ALREADY\_EXISTS}
	      \end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Responses}:
	      \begin{itemize}
		      ...
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_PATH\_ID}, \texttt{MISSING\_VISIBILITY}, \texttt{INVALID\_VISIBILITY}
		      \item \textbf{401 Unauthorized}: \texttt{USER\_NOT\_AUTHENTICATED}
...
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{204 No Content}
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_PATH\_ID}
		      \item \textbf{401 Unauthorized}: \texttt{USER\_NOT\_AUTHENTICATED}
		      \item \textbf{403 Forbidden}: \texttt{FORBIDDEN}
		      \item \textbf{404 Not Found}: \texttt{NOT\_FOUND}
	      \end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{201 Created}: \texttt{\{ success, message, data \}}
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_ORIGIN\_DESTINATION}, \texttt{MISSING\_DATES}, \texttt{INVALID\_DATES}, \texttt{INVALID\_DATE\_RANGE}, \texttt{MISSING\_TRIP\_SEGMENTS}, \texttt{DUPLICATE\_SEGMENT\_IDS}
		      \item \textbf{401 Unauthorized}: \texttt{USER\_NOT\_AUTHENTICATED}
	      \end{itemize}
\end{itemize}
...
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{204 No Content}
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_TRIP\_ID}
		      \item \textbf{401 Unauthorized}: \texttt{USER\_NOT\_AUTHENTICATED}
		      \item \textbf{403 Forbidden}: \texttt{FORBIDDEN}
		      \item \textbf{404 Not Found}: \texttt{NOT\_FOUND}
	      \end{itemize}
...
\paragraph{\textbf{GET} \texttt{/reports?pathId=:pathId}}
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, data \}} (empty list allowed)
...
\paragraph{\textbf{POST} \texttt{/reports}}
\begin{itemize}
	\item \textbf{Request Body}: \texttt{\{ pathId, position, reportType, description?, media? \}}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{201 Created}: \texttt{\{ success, message, data \}}
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_PATH\_ID}, \texttt{MISSING\_POSITION}, \texttt{INVALID\_REPORT\_TYPE}
		      \item \textbf{401 Unauthorized}: \texttt{USER\_NOT\_AUTHENTICATED}
		      \item \textbf{404 Not Found}: \texttt{PATH\_NOT\_FOUND}
	      \end{itemize}
\end{itemize}

\paragraph{\textbf{POST} \texttt{/reports/:reportId/confirm}}
\begin{itemize}
	\item \textbf{Request Body}: \texttt{\{ decision \}}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{201 Created}: \texttt{\{ success, message, data \}}
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_REPORT\_ID}, \texttt{INVALID\_DECISION}
		      \item \textbf{401 Unauthorized}: \texttt{USER\_NOT\_AUTHENTICATED}
		      \item \textbf{404 Not Found}: \texttt{REPORT\_NOT\_FOUND}
	      \end{itemize}
\end{itemize}

\subsubsection*{Stats}
\paragraph{\textbf{GET} \texttt{/stats/overall}}
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, data \}}
