...
If the data is valid, the mobile app sends a registration request to the \textbf{API Gateway}, which forwards it to the \textbf{AuthManager}.
The \textbf{AuthManager} checks whether the email is already in use by querying the DBMS through the \textbf{QueryManager}.
If the email already exists, the system returns an error and the mobile app notifies the user.

If the email is not already in use, the \textbf{AuthManager} stores the new user in the database (with the password handled securely) and generates an \textbf{authentication token}.
The token is returned to the mobile app, which stores it using the device \textbf{secure storage}.
...
When the trip is valid, the \textbf{TripManager} optionally retrieves a weather
...
confirm or dismiss the reported issue.
If the user ignores the notification, the form is closed after a short period of time
...
When validation succeeds, the app sends the confirmation request to the backend through
...
a confirmation entry associated with the selected report and the current user.
The confirmation is persisted through the \textbf{QueryManager} by inserting the record
into the \textbf{DBMS}.
...
A logged-in user wants to permanently delete one of his recored trips.
