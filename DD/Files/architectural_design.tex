...
The TripManager generates the summary, enriches it with environmental data retrieved from the Weather Manager,
...
\textbf{QueryManager} to retrieve pre-aggregated segment conditions (kept up to date by the ReportManager) alongside path data needed for route computation.
...
The \textbf{Statistics Manager} optimizes system performance by using a state-aware recomputation strategy that minimizes redundant processing. For overall statistics, it compares the user’s current total trip count against the value stored from the last calculation; it only triggers a full aggregation of data if the \textbf{QueryManager} reports a change in the trip count, otherwise it simply retrieves the previously stored results.

For individual trip statistics, the manager utilizes a lazy initialization pattern by asking the \textbf{QueryManager} if a processed record already exists for a specific trip ID. If the statistics are found, they are returned immediately; if not, the manager computes and persists them for the first time. This dual-layered approach ensures that data is only processed when necessary, effectively eliminating double computations by relying on the \textbf{QueryManager} to track the state of processed data.
...
portion of the path graph, including segment condition indicators derived from reports, from the
database using the \textbf{QueryManager}. Once the graph data is retrieved, the \textbf{PathManager}
computes the optimal path(s) according to the requested constraints. If valid routes are found,
the API Gateway returns them to the mobile app, which displays the corresponding suggestions.
...
When a logged-in user wants to create a new path, he will be presented with a form to fill
in the required metadata and choose between manual and automatic creation modes. If he opts
for creating the path in \textbf{manual mode}, an interactive map is displayed on the mobile app,
allowing the user to define the path by drawing it. The user will be able to
save the path, sending the saving request to the backend.
...
creation mode from the mobile app. The app displays a form for entering
the required metadata, such as the path name, description, and other relevant details, and
choosing the desired mode. If the user chooses automatic mode, 
the app proceeds to performs a first local validation. If the fields are invalid, the user is immediately notified.
...
When a logged-in user wants to delete one of his paths, he firstly navigates to the list of
...
In this scenario, a logged-in user starts a trip by selecting a path.
The interaction begins when the user initiates the trip from the mobile app, which
displays the chosen path on the map. The user chooses not to enable automatic mode,
...
The trip identifier is generated and maintained locally by the app during tracking and will be sent to the backend only when the trip is finalised.
...
The trip identifier remains local to the app during tracking and is forwarded to the backend only when the trip is stopped and finalised.
...
external sensor streams) and sends a stop-trip request to the backend, including the
locally maintained trip identifier.
The \textbf{TripManager} validates the request and retrieves the corresponding trip
...
If validation succeeds, the \textbf{TripManager} contacts the \textbf{WeatherManager}
to obtain a contextual weather snapshot. It then composes the final trip summary using
distance, duration, speed metrics, sensor-derived data (when available), and the weather snapshot.
The summary is then saved through the \textbf{QueryManager}.
...
including timestamps, distance, speed metrics, the associated path, and any stored weather snapshots.
...
completion, composes the final summary enriched with weather data supplied by the client after it retrieved a snapshot
from the \textbf{WeatherManager}. All persistent data is saved through the \textbf{QueryManager}.

The component exposes the following methods:

\begin{itemize}
	\item \textbf{finalizeTrip(tripId, tripPayload)}: receives the final trip payload from the API (including the locally
	...
	\item \textbf{getTripDetails(tripId, userId)}: verifies ownership of the requested trip and returns its full stored details.
\end{itemize}
...
\begin{itemize}
	\item \textbf{findPath(start, end, constraints)}: retrieves the required portion of the graph, including report-derived condition indicators
	      for each segmen, and computes candidate routes satisfying the given constraints.
	...
	\item \textbf{createPath(pathPayload)}: validates metadata and segment lists (manual mode) or GPS-derived samples (automatic mode), persists
...
\begin{itemize}
	\item \textbf{createReport(reportPayload)}: processes manual reports, automatic sensor-based reports, and confirmations of existing reports.
...
\begin{itemize}
	\item \textbf{computeOverallStats(userId)}: retrieves all trips owned by the user and computes cumulative indicators
	      such as total distance, total duration, and average speed.
	      % \item \textbf{aggregate(tripData)}: internal utility method used to consolidate raw trip rows into aggregated metrics, as shown in the Runtime View.
	\item \textbf{computeTripStats(userId, tripId)}: verifies ownership and computes per-trip statistics including average
	      pace, speed, and time splits.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{insertUser(userData)}: inserts a new user record with hashed credentials and stores a refresh token issued for the user.
	...
	\item \textbf{insertPath(pathPayload)}: stores new path metadata and geometry.
	\item \textbf{fetchUserPaths(userId)}: retrieves all paths created by the given user.
	\item \textbf{verifyOwnership(pathId, userId)}: checks whether the specified path belongs to the requesting user.
	...
	\item \textbf{insertReport(reportPayload)}: persists manual or automatic reports and confirmations.
	\item \textbf{insertTripSummary(tripSummary)}: inserts a completed trip summary with its associated weather snapshot.
	\item \textbf{fetchTrips(userId)}: retrieves all trips belonging to the user, ordered by date.
	\item \textbf{fetchTrip(tripId, userId)}: retrieves detailed trip data, enforcing ownership.
...
\begin{itemize}
	...
	\item \textbf{Service Layer pattern}.
	      ...
	      aggregates operations into meaningful methods,
	      and exposes a stable API to the Gateway.
