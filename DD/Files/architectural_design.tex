...
\begin{itemize}
	...
	to fetch and aggregate meteorological data.
	...
	\item \textbf{SnappingService}: maps raw segments to road geometry to improve
	      path accuracy before storage and analysis.
\end{itemize}
...
\begin{itemize}
	\item \textbf{AuthAPI}: endpoints for login, token refresh, and token validation.
...
The TripManager persists the trip data through the \textbf{QueryManager} and can enrich the stored record with environmental
data retrieved through the \textbf{WeatherService}.
...
\subsubsection{Weather Service}
The \textbf{Weather Service} wraps calls to an external provider and aggregates
...
\subsubsection{Geocoding Service}
The \textbf{Geocoding Service} wraps calls to an external provider to
...
\subsubsection{Snapping Service}
The \textbf{Snapping Service} interfaces with an internal routing engine to
...
\begin{itemize}
	...
	\item \textbf{Application Tier:}
	      ...
	      All external requests terminate at a shared \textit{NGINX} reverse proxy running on the server, which exposes the only public
	      entrypoint and forwards traffic to the backend containers over an internal Docker network.
	      TLS termination is handled at this reverse proxy using Cloudflare Origin Certificates, and the proxy also provides
	      rate limiting, request filtering, and load balancing across backend replicas.
	      The backend application itself is executed on one or more stateless Application Servers (Docker containers),
	      each running the BBP RESTful backend.
	      The snapping engine runs as a separate internal container and is reachable
	      only over the private Docker network.
...
is sent with a \textbf{PATCH} request to the \textbf{API Gateway},
...
Before applying the update, the \textbf{UserManager} checks for duplicate
email or username values to prevent conflicts.
...
to ensure that both fields are not empty. If the validation fails, the user is immediately notified.
...
The \textbf{API Gateway} forwards the request to the \textbf{ReportManager}, which retrieves
the corresponding reports by querying the database through the \textbf{QueryManager}.

If reports are found, the ReportManager returns them to the API Gateway,
...
sends a finalisation request to the backend through the \textbf{API Gateway}.
...
\textbf{StatsManager} to compute and aggregate trip statistics, which are stored
together with the trip data. The complete trip summary is persisted through the
\textbf{QueryManager}.
...
\begin{itemize}
	...
	\item \textbf{refreshToken(refreshToken)}: validates and rotates the refresh token, issuing a new access/refresh token pair.
...
\begin{itemize}
	...
	\item \textbf{getProfile(userId)}: retrieves the stored profile for the authenticated user.
...
\begin{itemize}
	\item \textbf{searchPath(origin, destination, userId?)}: geocodes origin/destination, filters existing paths by visibility and proximity, and returns matching routes.
	\item \textbf{createPath(pathPayload, userId)}: validates metadata and path segments (manual mode only), persists the new path, and associates it with the creator.
	\item \textbf{snapPath(coordinates)}: snaps drawn coordinates to the road network using an external routing service.
...
\begin{itemize}
	\item \textbf{createTrip(tripPayload, userId)}: stores a completed trip using origin/destination, timestamps, and trip segment references.
...
\textbf{TripManager} to enrich trip summaries with environmental context.

\begin{itemize}
	\item \textbf{fetchAndAggregateWeatherData(coordinates)}: samples coordinates along the trip route, fetches point data, and returns an aggregated snapshot.
\end{itemize}
...
\begin{itemize}
	\item \textbf{createUser(email, password, username, systemPreferences)}: creates a new user record.
	\item \textbf{getUserByEmail(email)}: retrieves a user by email.
	\item \textbf{getUserById(userId)}: retrieves a user by identifier.
	\item \textbf{getUserByUsername(username)}: retrieves a user by username.
	\item \textbf{updateUserProfile(userId, profilePayload)}: stores changes to user profile fields.
	\item \textbf{createRefreshToken(userId, token, expiresAt)}: persists a refresh token.
	\item \textbf{getRefreshToken(token)}: retrieves a refresh token.
	\item \textbf{deleteRefreshToken(token)}: removes a refresh token.
	\item \textbf{getTripById(tripId)}: retrieves a trip and its ordered segments.
	\item \textbf{createTrip(userId, origin, destination, startedAt, finishedAt, statistics, tripSegments, title)}: stores a completed trip.
	\item \textbf{getTripsByUserId(userId)}: retrieves all trips for a user, ordered by date.
	\item \textbf{deleteTripById(tripId)}: deletes a trip.
	\item \textbf{updateTripWeather(tripId, weather)}: stores aggregated weather data for a trip.
	\item \textbf{createPath(userId, origin, destination, pathSegments, visibility, creationMode, title, description, distanceKm)}: stores a new path.
	\item \textbf{getPathById(pathId)}: retrieves a path and its ordered segments.
	\item \textbf{getPathsByUserId(userId)}: retrieves all paths for a user.
	\item \textbf{getPathByOriginDestination(userId, origin, destination)}: finds a user path by origin/destination.
	\item \textbf{searchPathsByOriginDestination(origin, destination, userId?)}: returns matching paths by proximity and visibility.
	\item \textbf{updatePathScore(pathId, score)}: updates the stored path score.
	\item \textbf{updatePathStatus(pathId, status)}: updates the stored path status.
	\item \textbf{deletePathById(pathId)}: deletes a path.
	\item \textbf{changePathVisibility(pathId, visibility)}: updates path visibility.
	\item \textbf{createSegment(status, polylineCoordinates)}: stores a segment.
	\item \textbf{createSegmentWithId(segmentId, status, polylineCoordinates)}: stores a segment with a provided id.
	\item \textbf{getSegmentsByIds(segmentIds)}: retrieves segments by id.
	\item \textbf{getSegmentStatistics(segmentIds)}: retrieves segment status for scoring.
\end{itemize}
...
All endpoints exchange JSON payloads over HTTPS.
...
\paragraph{\textbf{POST} \texttt{/auth/refresh}}
\begin{itemize}
	\item \textbf{Request Body}: \texttt{\{ refreshToken \}}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, message, tokens \}}
		      \item \textbf{401 Unauthorized}: \texttt{MISSING\_REFRESH\_TOKEN}
		      \item \textbf{403 Forbidden}: \texttt{INVALID\_REFRESH\_TOKEN}, \texttt{REFRESH\_TOKEN\_NOT\_FOUND}, \texttt{REFRESH\_TOKEN\_EXPIRED}
		      \item \textbf{404 Not Found}: \texttt{USER\_NOT\_FOUND}
	      \end{itemize}
\end{itemize}

\subsubsection*{Users}
\paragraph{\textbf{POST} \texttt{/users/register}}
\begin{itemize}
...
\paragraph{\textbf{GET} \texttt{/users/me}}
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, data \}}
		      \item \textbf{401 Unauthorized}: \texttt{USER\_NOT\_AUTHENTICATED}
		      \item \textbf{404 Not Found}: \texttt{USER\_NOT\_FOUND}
	      \end{itemize}
\end{itemize}

\paragraph{\textbf{PATCH} \texttt{/users/me}}
\begin{itemize}
	...
	\item \textbf{Responses}:
	      \begin{itemize}
		      ...
		      \item \textbf{400 Bad Request}: \texttt{EMPTY\_PAYLOAD}, \texttt{INCORRECT\_PASSWORD}
		      \item \textbf{401 Unauthorized}: \texttt{USER\_NOT\_AUTHENTICATED}
		      \item \textbf{404 Not Found}: \texttt{USER\_NOT\_FOUND}
		      \item \textbf{409 Conflict}: \texttt{EMAIL\_ALREADY\_IN\_USE}, \texttt{USERNAME\_ALREADY\_IN\_USE}
	      \end{itemize}
...
\subsubsection*{Paths}
\paragraph{\textbf{POST} \texttt{/paths}}
\begin{itemize}
	...
	\item \textbf{Responses}:
	      \begin{itemize}
		      ...
		      \item \textbf{400 Bad Request}: \texttt{UNAUTHORIZED}, \texttt{MISSING\_PATH\_SEGMENTS}, \texttt{MISSING\_VISIBILITY}, \texttt{INVALID\_SEGMENT}, \texttt{MISSING\_CREATION\_MODE}, \texttt{INVALID\_CREATION\_MODE}, \texttt{NOT\_IMPLEMENTED}
		      \item \textbf{404 Not Found}: \texttt{PATH\_NOT\_FOUND\_AFTER\_CREATION}
		      \item \textbf{409 Conflict}: \texttt{Path with same origin and destination already exists}
	      \end{itemize}
\end{itemize}

\paragraph{\textbf{GET} \texttt{/paths/search}}
\begin{itemize}
	\item \textbf{Query Params}: \texttt{origin}, \texttt{destination}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, data \}}
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_ORIGIN}, \texttt{MISSING\_DESTINATION}
		      \item \textbf{404 Not Found}: \texttt{NO\_ROUTE}
	      \end{itemize}
\end{itemize}

\paragraph{\textbf{POST} \texttt{/paths/snap}}
\begin{itemize}
	\item \textbf{Request Body}: \texttt{\{ coordinates \}}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, data \}}
		      \item \textbf{400 Bad Request}: \texttt{MISSING\_COORDINATES}
	      \end{itemize}
\end{itemize}

\paragraph{\textbf{GET} \texttt{/paths?owner=me}}
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, data \}} (empty list allowed)
		      \item \textbf{400 Bad Request}: \texttt{UNAUTHORIZED}
	      \end{itemize}
\end{itemize}

\paragraph{\textbf{PATCH} \texttt{/paths/:pathId/visibility}}
\begin{itemize}
	\item \textbf{Request Body}: \texttt{\{ visibility \}}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, message, data \}}
		      \item \textbf{400 Bad Request}: \texttt{UNAUTHORIZED}, \texttt{MISSING\_PATH\_ID}, \texttt{MISSING\_VISIBILITY}, \texttt{INVALID\_VISIBILITY}
		      \item \textbf{403 Forbidden}: \texttt{FORBIDDEN}
		      \item \textbf{404 Not Found}: \texttt{NOT\_FOUND}
	      \end{itemize}
\end{itemize}

\paragraph{\textbf{DELETE} \texttt{/paths/:pathId}}
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{204 No Content}
		      \item \textbf{400 Bad Request}: \texttt{UNAUTHORIZED}, \texttt{MISSING\_PATH\_ID}
		      \item \textbf{403 Forbidden}: \texttt{FORBIDDEN}
		      \item \textbf{404 Not Found}: \texttt{NOT\_FOUND}
	      \end{itemize}
\end{itemize}
...
\subsubsection*{Trips}
\paragraph{\textbf{POST} \texttt{/trips}}
\begin{itemize}
	\item \textbf{Request Body}: \texttt{\{ origin, destination, startedAt, finishedAt, tripSegments, title? \}}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{201 Created}: \texttt{\{ success, message, data \}}
		      \item \textbf{400 Bad Request}: \texttt{UNAUTHORIZED}, \texttt{MISSING\_ORIGIN\_DESTINATION}, \texttt{MISSING\_DATES}, \texttt{INVALID\_DATES}, \texttt{INVALID\_DATE\_RANGE}, \texttt{MISSING\_TRIP\_SEGMENTS}, \texttt{DUPLICATE\_SEGMENT\_IDS}
	      \end{itemize}
\end{itemize}

\paragraph{\textbf{GET} \texttt{/trips?owner=me}}
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{200 OK}: \texttt{\{ success, data \}} (empty list allowed)
...
\paragraph{\textbf{DELETE} \texttt{/trips/:tripId}}
\begin{itemize}
	\item \textbf{Responses}:
	      \begin{itemize}
		      \item \textbf{204 No Content}
		      \item \textbf{400 Bad Request}: \texttt{UNAUTHORIZED}, \texttt{MISSING\_TRIP\_ID}
		      \item \textbf{403 Forbidden}: \texttt{FORBIDDEN}
		      \item \textbf{404 Not Found}: \texttt{NOT\_FOUND}
	      \end{itemize}
\end{itemize}
