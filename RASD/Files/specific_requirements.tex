...
\begin{enumerate}
	...
	\item \textbf{Trip Recording and Management}
	      \begin{itemize}
		      \item[\textbf{[R6]}] The system shall allow logged-in users to start a cycling trip in manual or automatic mode.
		      \item[\textbf{[R7]}] The system shall allow logged-in users to stop a currently active trip and save the recorded data.
		      \item[\textbf{[R8]}] The system shall collect GPS data during trip recording.
		      \item[\textbf{[R9]}] The system shall collect motion sensor data (accelerometer, gyroscope) during trip recording when available.
		      \item[\textbf{[R10]}] The system shall allow logged-in users to view the list of their recorded trips.
		      \item[\textbf{[R11]}] The system shall allow logged-in users to view a summary of their overall cycling statistics (total distance, total time, average speed, etc.).
		      \item[\textbf{[R12]}] The system shall allow logged-in users to view statistics for each trip (distance, speed, duration, etc.).
		      \item[\textbf{[R13]}] The system shall display the route and reported obstacles associated with a recorded trip.
		      \item[\textbf{[R14]}] The system shall allow logged-in users to delete a recorded trip.
	      \end{itemize}
	\item \textbf{Trip Data Enrichment}
	      \begin{itemize}
		      \item[\textbf{[R15]}] The system shall communicate with external weather services to retrieve meteorological data related to the time and location of a trip.
	      \end{itemize}
	\item \textbf{Automatic Data Processing and Detection}
	      \begin{itemize}
		      \item[\textbf{[R16]}] The system shall detect when a user is cycling based on speed and acceleration patterns.
		      \item[\textbf{[R17]}] The system shall detect irregular movements from sensor data that may suggest potholes or surface defects.
		      \item[\textbf{[R18]}] The system shall present automatically detected path and obstacle data to the user for manual confirmation before publishing.
	      \end{itemize}
	\item \textbf{Path Information Management}
	      \begin{itemize}
		      \item[\textbf{[R19]}] The system shall allow logged-in users to manually create a new bike path by drawing or specifying street segments.
		      \item[\textbf{[R20]}] The system shall allow logged-in users to manually report obstacles or problems on a bike path.
		      \item[\textbf{[R21]}] The system shall allow logged-in users to manually confirm or reject the presence of obstacles reported by other users.
		      \item[\textbf{[R22]}] The system shall allow logged-in users to create a new bike path in automatic mode using GPS tracking.
		      \item[\textbf{[R23]}] The system shall allow logged-in users to delete or edit their previously created paths.
		      \item[\textbf{[R24]}] The system shall allow logged-in users to set the visibility of their created paths as public or private.
	      \end{itemize}

	\item \textbf{Path Data Aggregation and Status Evaluation}
	      \begin{itemize}
		      \item[\textbf{[R25]}] The system shall aggregate multiple user reports referring to the same path segment.
		      \item[\textbf{[R26]}] The system shall evaluate the reliability of each path segment based on the number of confirmations and report freshness.
		      \item[\textbf{[R27]}] The system shall determine the current status of a path (optimal, medium, sufficient, requires maintenance, closed).
		      \item[\textbf{[R28]}] The system shall allow any user (guest or logged-in) to view the detailed status and latest reports of a selected bike path.
	      \end{itemize}
	\item \textbf{Route Discovery and Search}
	      \begin{itemize}
		      \item[\textbf{[R29]}] The system shall allow any user (guest or logged-in) to browse available public bike paths on a map.
		      \item[\textbf{[R30]}] The system shall allow any user to search for bike paths connecting two locations.
		      \item[\textbf{[R31]}] The system shall compute suggested routes based on path quality and distance.
		      \item[\textbf{[R32]}] The system shall rank suggested routes according to their safety and quality.
	      \end{itemize}
	\item \textbf{Navigation}
	      \begin{itemize}
		      \item[\textbf{[R33]}] The system shall display the user’s current GPS position during navigation along a selected path.
	      \end{itemize}
	\item \textbf{Notifications and Alerts}
	      \begin{itemize}
		      \item[\textbf{[R34]}] The system shall send in-app notifications or pop-ups to warn users about nearby obstacles or closed path segments during navigation.
	      \end{itemize}
	\item \textbf{External Services Integration}
	      \begin{itemize}
		      \item[\textbf{[R35]}] The system shall interface with map and geocoding services to translate addresses into coordinates and render paths.
		      \item[\textbf{[R36]}] The system shall ensure that communication with all external services (map, weather) handles temporary unavailability gracefully.
	      \end{itemize}
\end{enumerate}
This subsection specifies all the functional requirements that the BBP system must fulfill to achieve the goals described in the previous chapters.
The requirements are organized according to the main functionalities of the system and are expressed in a testable form, describing what the system shall do rather than how it shall be implemented.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		\hline
		\textbf{Actor(s)}        & Logged-in User                                                                                                      \\ \hline
		\textbf{Entry Condition} & The actor is authenticated, has selected a path to follow, and wants to start the trip with manual mode.            \\ \hline
		\textbf{Event Flow}      & 1. The actor chooses to start the trip on the selected path.                                                        \\
		                         & 2. The system displays the available start modes.                                                                   \\
		                         & 3. The actor selects Manual mode.                                                                                   \\
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		\hline
		\textbf{Actor(s)}        & Logged-in User                                                                                                                                                                                \\ \hline
		\textbf{Entry Condition} & The actor is authenticated, has selected a path to follow, and wants to start the trip in Automatic mode, using external devices.                                                             \\ \hline
		\textbf{Event Flow}      & 1. The actor chooses to start the trip on the selected path.                                                                                                                                  \\
		                         & 2. The system displays the available start modes.                                                                                                                                             \\
		                         & 3. The actor selects Automatic mode.                                                                                                                                                          \\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{Images/SequenceDiagrams/report_confirm.png}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor accesses the “Trip History” section from the profile or main menu.            \\
		                         & 3. The system displays the trips associated with the actor chronologically.                \\
		                         & 4. The actor can selects a specific trip to view detailed data (map, statistics, reports). \\
		                         & 5. The system retrieves and displays the detailed information for the selected trip.       \\ \hline
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Exceptions}      & IIf no trip data are available, the system shows an error message.                                                                \\
		                         & If a temporary connection or BBP error occurs, the system displays an error message and allows retry.                             \\ \hline
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|l|X|p{0.3\textwidth}|}
		...
		\textbf{G1}   & R5, R25, R26, R27, R28, R29, R30, R31, R32, R35, R36                      & D1, D5, D6, D8              \\ \hline
		\textbf{G2}   & R5, R25, R26, R27, R28, R29, R30, R31, R32, R35, R36                      & D1, D5, D6, D8              \\ \hline
		\textbf{G3}   & R5, R8, R9, R25, R28, R29, R30, R33, R34, R35, R36                   & D1, D3, D4, D6, D7, D8      \\ \hline
		\textbf{G4}   & R1, R2, R3, R4, R6, R7, R8, R9, R10, R13, R14, R15, R16, R35, R36         & D1, D2, D3, D4, D6          \\ \hline
		\textbf{G5}   & R1, R2, R6, R8, R9, R16, R17, R18, R19, R20, R21, R22, R23, R24, R35, R36 & D1. D3, D4, D5, D6, D7, D8  \\ \hline
		\textbf{G6}   & R1, R2, R3, R4, R6, R7, R8, R9, R10, R11, R12, R15, R16, R35, R36         & D1. D2, D3, D4, D6          \\ \hline
	\end{tabularx}
