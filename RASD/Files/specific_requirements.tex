...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{Images/SequenceDiagrams/delete_path.png}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.3\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor chooses to start the selected path.                                                                                         \\
		                         & 2. The system initializes the trip visualization and displays the path on the map.                                                       \\
		                         & 3. The actor activates sends real-time location data.                                                                                    \\
		...
		\textbf{Exceptions}      & If the GPS signal or connection to BBP fails, the system displays an error message and allows retry.                                     \\ \hline
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{Images/SequenceDiagrams/start_trip_guest.png}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.3\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor chooses to start the selected path.                                                                                                                           \\
		                         ...
		                         & \hspace{1em} \textbf{Automatic mode:} the system connects to external devices (e.g., GPS, gyroscope, accelerometer) for real-time data collection and automatic reporting. \\
		                         & \hspace{1em} \textbf{Manual mode:} the system enables manual entry of relevant trip data (e.g., condition reports, notes).                                                 \\
		                         & 4. The system initializes the chosen mode and gets the GPS position.                                                                                                       \\
		                         & 5. The system starts the trip and begins data collection.                                                                                                                  \\ \hline
		\textbf{Exit Condition}  & The trip is successfully started; the chosen recording mode is active.                                                                                                     \\ \hline
		\textbf{Exceptions}      & If GPS or sensor services are unavailable, the system displays an error and allows switching to Manual mode.                                                               \\
		                         & If a temporary connection or BBP error occurs, the system displays an error message and allows retry.                                                                      \\ \hline
	...
	\end{tabularx}
	\caption{Start a Trip (Logged-in User) — Process Detail}
\end{table}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{Images/SequenceDiagrams/start_trip_loggedin.png}
	\caption{Start a Trip (Logged-in User) — Sequence Diagram}
	\label{fig:use_case_start_trip_loggedin}
\end{figure}
