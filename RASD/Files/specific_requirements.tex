...
\begin{enumerate}
	...
	\item \textbf{Trip Recording and Management}
	      \begin{itemize}
		      ...
		      \item[\textbf{[R15]}] The system shall collect motion sensor data (accelerometer, gyroscope) during trip recording only when Automatic Mode is enabled.
	...
	\item \textbf{Automatic Data Processing and Detection}
	      \begin{itemize}
		      ...
		      \item[\textbf{[R23]}] The system shall detect irregular movements from sensor data that may suggest potholes or surface defects when Automatic Mode is enabled.
	...
	\item \textbf{Path Information Management}
	      \begin{itemize}
		      \item[\textbf{[R25]}] The system shall allow logged-in users to manually create a new bike path by drawing segments.
	...
	\item \textbf{Alerts}
	      \begin{itemize}
		      \item[\textbf{[R40]}] The system shall send pop-ups to warn users about nearby obstacles or closed path segments during an active trip.
	      \end{itemize}
...
Once authenticated, users can log out, edit their personal information, and manage their profile settings.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{Images/Use_Case_Diagrams/register_login_uc.png}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor clicks on the “Sign Up” button.                                                          \\
		                         ...
		                         & 5. If validation succeeds, the system creates a new account and confirms the successful registration. \\ \hline
		...
		\textbf{Exceptions}      & If the data are invalid or incomplete, the system shows an error and asks for correction.             \\
		                         & If the email or username is already in use, the system shows an error.                                \\
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor clicks the “Log In” button.                                                                                \\
		                         ...
		                         & 5. If validation succeeds, the system authenticates the user.                                                           \\ \hline
		...
		\textbf{Notes}           & After login, a valid session is created.                                                                                \\ \hline
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Exceptions}      & If BBP cannot invalidate the session or the network is unavailable, the actor remains logged out locally; server revocation can be retried later. \\ \hline
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor selects the “Edit Profile” action.                                                                                                                \\
		                         & 2. The system displays the editable profile form (e.g., username, email).                                                                                      \\
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor enters start and destination points.                                                       \\
		                         & 2. The system validates the provided inputs.                                                            \\
		                         & 3. The system displays the list of suggested paths to the actor.                                        \\ \hline
		\textbf{Exit Condition}  & The suggested paths are displayed to the actor.                                                         \\ \hline
		\textbf{Exceptions}      & If the inputs are invalid or incomplete, the system displays an error message.                          \\
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		\hline
		\textbf{Actor(s)}        & Guest User, Logged-in User                                                                                                   \\ \hline
		\textbf{Entry Condition} & The actor has just performed a search and is browsing the results of suggested paths.                                        \\ \hline
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor requests the creation of a new path.                                           \\
		                         & 2. The system displays an input page for path metadata and creation mode selection.         \\
		                         & 3. The actor submits the required metadata and selects the manual creation mode.            \\
		                         & 4. The system validates the provided metadata.                                              \\
		                         & 5. If validation succeeds, the system displays a map interface to manually define the path. \\
		                         & 6. The actor submits the drawn path segments.                                               \\
		                         & 7. The system saves the new path.                                                           \\ \hline
		\textbf{Exit Condition}  & The new path is successfully created and stored by the system.                              \\ \hline
		\textbf{Exceptions}      & If the provided data are invalid or incomplete, the system displays an error message.       \\
		                         & If a temporary connection or BBP error occurs, the system displays an error message.        \\ \hline
		\textbf{Notes}           & The manually created path is associated with the actor who created it.                      \\ \hline
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor selects the creation button.                                                                                                      \\
		                         & 2. The system displays an input page for path metadata and creation mode selection.                                                            \\
		                         & 3. The actor submits the required metadata and selects the automatic creation mode.                                                            \\
		                         & 4. The system validates the provided metadata.                                                                                                 \\
		                         & 5. If validation succeeds, the system starts GPS tracking.                                                                                     \\
		                         & 6. The system continuously collects position data while tracking is active.                                                                    \\
		                         & 7. When tracking stops, the system validates the collected position data.                                                                      \\
		                         & 8. If validation succeeds, the system saves the new path.                                                                                      \\ \hline
		\textbf{Exit Condition}  & The automatically recorded path is successfully created and stored by the system.                                                              \\ \hline
		\textbf{Exceptions}      & If the provided data are invalid or incomplete, the system displays an error message.                                                          \\
		                         & If a GPS error occurs during tracking, the system displays an error message.                                                                   \\
		                         & If a temporary connection or BBP error occurs, the system displays an error message.                                                           \\ \hline
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		\hline
		\textbf{Actor(s)}        & Logged-in User                                                                               \\ \hline
		\textbf{Entry Condition} & The actor is authenticated and wants to view their personal path library.                    \\ \hline
		\textbf{Event Flow}      & 1. The actor requests to view their saved paths.                                             \\
		                         & 2. If the request succeeds, the system displays the list of paths associated with the actor. \\ \hline
		\textbf{Exit Condition}  & The actor’s personal path library is displayed.                                              \\ \hline
		\textbf{Exceptions}      & If no paths are found, the system displays an empty list message.                             \\
		                         & If a temporary connection or BBP error occurs, the system displays an error message.         \\ \hline
		\textbf{Notes}           & Only paths created or saved by the authenticated user are shown.                             \\ \hline
	\end{tabularx}
	\caption{View Path Library Process Detail}
\end{table}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Images/SequenceDiagrams/path_library.png}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor requests to change the visibility of a selected path.                                                       \\
		                         & 2. If the request succeeds, the system updates the visibility of the path and confirms the operation.                    \\ \hline
		\textbf{Exit Condition}  & The visibility status of the selected path is successfully updated.                                                      \\ \hline
		\textbf{Exceptions}      & If the actor is not the owner of the selected path, the system displays an error message.                                \\
		                         & If the selected path is not found, the system displays an error message.                                                 \\
		                         & If a temporary connection or BBP error occurs, the system displays an error message.                                     \\ \hline
		\textbf{Notes}           & Only the owner of a path is allowed to modify its visibility.                                                            \\ \hline
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		\hline
		\textbf{Actor(s)}        & Logged-in User                                                                            \\ \hline
		\textbf{Entry Condition} & The actor is authenticated and wants to delete one of their previously created paths.     \\ \hline
		\textbf{Event Flow}      & 1. The actor requests the deletion of a selected path.                                    \\
		                         & 2. If the request succeeds, the system deletes the path and confirms the operation.       \\ \hline
		\textbf{Exit Condition}  & The selected path is successfully removed from the system.                                \\ \hline
		\textbf{Exceptions}      & If the actor is not the owner of the selected path, the system displays an error message. \\
		                         & If the selected path is not found, the system displays an error message.                  \\
		                         & If a temporary connection or BBP error occurs, the system displays an error message.      \\ \hline
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor chooses to start the trip on the selected path.                                                                \\
		                         & 2. The system displays the path on the map.                                                                                 \\
		                         & 3. The system starts GPS tracking.                                                                                          \\
		                         & 4. While the trip is active, the system receives position updates from the GPS and updates the actor's position on the map. \\ \hline
		...
		\textbf{Notes}           & Guest Users can only visualize their current location, no trip data are stored.                                             \\ \hline
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor chooses to start the trip on the selected path.                                                                                    \\
		                         & 2. The system prompts the actor to enable automatic mode.                                                                                       \\
		                         & 3. The actor declines the automatic mode prompt.                                                                                                \\
		                         & 4. The system activates GPS tracking.                                                                                                           \\
		                         & 5. While the trip is active, the system receives position updates from the GPS and updates the actor's position on the map.                     \\ \hline
		...
		\textbf{Notes}           & While the trip is active, the system records trip data (e.g., time, distance, speed) for the actor's statistics.                                \\ \hline
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor chooses to start the trip on the selected path.                                                                                                                                  \\
		                         & 2. The system prompts the actor to enable automatic mode.                                                                                                                                     \\
		                         & 3. The actor enables automatic mode.                                                                                                                                                          \\
		                         & 4. The system attempts to connect to external devices.                                                                                                                                        \\
		                         & 5. If the connection succeeds, the system starts GPS tracking.                                                                                                                                \\
		                         & 6. While the trip is active, the system updates the actor's position on the map and records trip samples.                                                                                     \\ \hline
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor chooses to stop the trip, leaves the app or deviates from the path.         \\
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. The actor chooses to stop the current trip, leaves the app or deviates from the path.                                                            \\
		                         ...
		                         & 4. If possible, the system enriches the trip data with meteorological information (e.g., weather, temperature), retrieved from an external service. \\
		                         & 5. The system stores the trip data in the actor's account.                                                                                          \\
		                         & 6. The system displays a success message.                                                                                                           \\ \hline
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Exceptions}      & If the data are invalid or incomplete, the system displays an error message and prevents submission until corrected.               \\
		                         & If the GPS position cannot be determined, the system displays an error message.                                                    \\
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		...
		\textbf{Event Flow}      & 1. While following the path, the system detects proximity to a segment with existing reports based on the actor’s current position.                                                            \\
		                         & 2. The system displays a popup summarizing the reported issue.                                                                                                                                 \\
		                         & 3. The actor chooses one of the available options:                                                                                                                                             \\
		                         & \hspace{0.5em} Confirm: the system registers the actor’s confirmation, increasing the report’s reliability.                                                                                    \\
		                         & \hspace{0.5em} Reject: the system registers the actor’s feedback as “not confirmed”.                                                                                                           \\
		                         & \hspace{0.5em} Ignore: the popup is dismissed without any action.                                                                                                                              \\
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		\hline
		\textbf{Actor(s)}        & Logged-in User                                                                                                \\ \hline
		\textbf{Entry Condition} & The actor is authenticated and requests to view their trip history.                                           \\ \hline
		\textbf{Event Flow}      & 1. The actor opens the trip history section.                                                                  \\
		                         & 2. The system retrieves the list of trips associated with the actor.                                          \\
		                         & 3. The system displays the trips list to the actor.                                                           \\ \hline
		\textbf{Exit Condition}  & The trips list is displayed to the actor.                                                                     \\ \hline
		\textbf{Exceptions}      & If no trips are found, the system displays an error message indicating that no trips are available.           \\
		                         & If a temporary connection problem or BBP error occurs, the system displays an error message and allows retry. \\ \hline
		\textbf{Notes}           & The system may support sorting or filtering of the trips list (e.g., by date, distance).                      \\ \hline
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		\hline
		\textbf{Actor(s)}        & Logged-in User                                                                                        \\ \hline
		\textbf{Entry Condition} & The actor is authenticated and has at least one stored trip.                                          \\ \hline
		\textbf{Event Flow}      & 1. The actor selects a trip from the trip history.                                                    \\
		                         & 2. The actor chooses to delete the selected trip.                                                     \\
		                         & 3. The system verifies ownership of the trip.                                                         \\
		                         & 4. The system deletes the trip and its associated data.                                               \\
		                         & 5. The system confirms the successful deletion.                                                       \\ \hline
		\textbf{Exit Condition}  & The selected trip and all its related data are permanently removed from the system.                   \\ \hline
		\textbf{Exceptions}      & If the trip does not exist or does not belong to the actor, the system displays an error message.     \\
		                         & If a temporary connection or BBP error occurs, the system displays an error message and allows retry. \\ \hline
		\textbf{Notes}           & Deleted trips cannot be restored. Deletion affects statistics and reports associated with the trip.   \\ \hline
	\end{tabularx}
	\caption{Delete a Trip Process Detail}
\end{table}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Images/SequenceDiagrams/delete_trip.png}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		\hline
		\textbf{Actor(s)}        & Logged-in User                                                                                        \\ \hline
		\textbf{Entry Condition} & The actor is authenticated and has selected a previously completed trip.                              \\ \hline
		\textbf{Event Flow}      & 1. The actor selects a specific trip from the trip history.                                           \\
		                         & 2. The system computes or retrieves statistics related to the selected trip.                          \\
		                         & 3. The system displays detailed statistics (e.g., distance, duration, speed, reports).                \\ \hline
		\textbf{Exit Condition}  & The actor can visualize the statistics associated with the selected trip.                             \\ \hline
		\textbf{Exceptions}      & If no data are available for the selected trip, the system displays an informative message.           \\
		                         & If a temporary connection or BBP error occurs, the system displays an error message and allows retry. \\ \hline
		\textbf{Notes}           & Trip statistics are computed from stored trip samples and validated data only.                        \\ \hline
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|}
		\hline
		\textbf{Actor(s)}        & Logged-in User                                                                                  \\ \hline
		\textbf{Entry Condition} & The actor is authenticated.                                                                     \\ \hline
		\textbf{Event Flow}      & 1. The actor accesses the statistics section.                                                   \\
		                         & 2. The system computes or retrieves overall statistics based on all stored trips.               \\
		                         & 3. The system displays aggregated statistics (e.g., total distance, total time, average speed). \\ \hline
		\textbf{Exit Condition}  & The actor can visualize the overall statistics of his trips.                                    \\ \hline
		\textbf{Exceptions}      & If no trips are available, the system displays a message indicating that no statistics exist.   \\
		                         & If a temporary connection or BBP error occurs, the system displays an error message.            \\ \hline
		\textbf{Notes}           & Statistics are computed from validated and stored trips only.                                   \\ \hline
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|l|X|p{0.3\textwidth}|}
		...
		\textbf{G1}   & R5, R31, R32, R33, R34, R35, R36, R37, R38, R41, R42                           & D1, D5, D6, D8, D9          \\ \hline
		\textbf{G2}   & R5, R31, R32, R33, R34, R35, R36, R37, R38, R41, R42                           & D1, D5, D6, D8, D9          \\ \hline
		\textbf{G3}   & R5, R6, R7, R8, R9, R10, R11, R14, R15, R31, R34, R35, R36, R39, R40, R41, R42 & D1, D3, D4, D6, D7, D8, D9  \\ \hline
...
The system shall provide timely feedback to user interactions.
Under normal network conditions, core functionalities such as map visualization, path search, and path selection shall return a result within 5 seconds.
User actions that require immediate feedback, such as starting or stopping a trip, submitting a report, or confirming a report, shall be acknowledged within 2 seconds.

During an active trip, the system shall support continuous tracking of the user’s position.
Location updates shall be processed at least every 2 seconds to enable a smooth visualization of movement along the selected path.

The system shall support multiple users performing common operations concurrently without significant degradation of response times.
Under normal operating conditions, the system shall be able to handle at least 200 concurrent users executing standard actions such as browsing paths, starting trips, and submitting reports.
