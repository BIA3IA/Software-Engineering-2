...
\begin{itemize}
	\item \textbf{Visual Studio Code}: editing of source code and documentation (LaTeX), with project-wide search and formatting support.
	\item \textbf{LaTeX}: typesetting system used to produce the final RASD document in a consistent format.
	\item \textbf{Git}: version control used to track changes and support collaborative development.
	\item \textbf{GitHub}: remote repository hosting and collaboration platform used for versioning, reviews, and issue tracking.
	\item \textbf{Lucidchart}: creation of UML diagrams (use case diagrams, state diagrams, domain class diagram).
	\item \textbf{Alloy TODO}: execution of the Alloy model, trace generation, and assertion checking.
\end{itemize}
