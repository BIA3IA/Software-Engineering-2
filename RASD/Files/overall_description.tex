...
\begin{enumerate}
	...
	\item \textbf{A logged-in user starts a trip}:
	      ...
	      Cesare decides to start a new bike trip. After selecting one of the suggested bike paths, he presses the Start button. At this point, BBP asks him whether he wants to enable the Automatic Mode.
	...
	\item \textbf{A logged-in user browse his cycling activities}:
	      ...
	      Each activity is accompanied by summary information such as the date, distance traveled, duration, and  destination.
	...
	\item \textbf{A logged-in user access his trip and per trip statistics}:
	      ...
	      Pietro selects his latest trip from his activity list. BBP opens the path details page, displaying the entire path on a map.
	...
	\item \textbf{A logged-in user forgets to start a trip}:
	      ...
	      After detecting that Eugenia is likely biking, the system shows an pop-up alert, asking if she wants to start recording her trip.
	      ...
	      At the end of her ride, Eugenia will find the complete trip summary in her activity history, including distance, duration, speed, and weather information retrieved from the external meteorological service.
...
The Domain Class Diagram for the Best Bike Paths (BBP) system illustrates the
key entities and their relationships for managing bike paths, trips, and associated information in a unified manner.

The core functionality revolves around the User, who can either
register to record activities and contribute data, or remain a guest and
only browse path information. A User is defined by identifiers such as
userId and email, along with their current position represented as a GeoPoint.

The central object of the system is the Path, which is defined as a
sequence of PathSegments linking an origin to a destination GeoPoint.
Each Path has a status (from the PathStatus enumeration), a computed
score, and a boolean flag indicating if it is public. A PathSegment is an
atomic portion of a path, identified by a segmentId and a
streetName, and maintains a reference to the next segment in the path.

Logged-in Users may record a Trip that follows a specific Path.
A Trip includes temporal information (startTime, endTime) and
metrics such as totalDistanceTraveled. Performance for each trip is
summarised in the associated Statistics entity, which records values
such as avgSpeed, maxSpeed, and other relevant indicators.
To enrich the user experience, a Trip may incorporate WeatherInfo,
including temperature, humidity, and a weather condition (WeatherType),
which is obtained from an external WeatherProvider.

Data about paths, especially regarding obstacles such as potholes, is collected
through Report entities. A User creates a Report using a specific
AcquisitionMode (MANUAL or AUTOMATIC). In the
automatic mode, the system uses raw data from Sensors (e.g.,
accelerometer, gyroscope) to detect and suggest possible issues. Each Report
specifies an ObstacleType and may include a statusSuggestion.

To maintain accuracy and reduce false positives, each Report must receive one
or more Confirmations from users, which validate the information with
a boolean value. Confirmations may also be used to validate individual PathSegments.

Finally, the system aggregates multiple Reports about a Path into a
MergedReport. This entity provides a consolidated view of the path's
condition, including the resulting status, the confirmationCount that
supports it, and a freshness interval (TimeFrame). All temporal data
within the system is represented using the DateTime entity, which stores values in epoch time for simplicity.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Images/State_Diagrams/selecting_trip.png}
...
% TODO review this image and process
...
\begin{itemize}
	\item \textbf{User Registration and Login}:
	      ...
	      Guest Users can browse available bike paths, search for routes between two locations, and receive path suggestions based on community rankings, but they cannot record trips, create new paths, or contribute reports.
	...
	\item \textbf{Bike Path Search and Visualization}:
	      ...
	      Users can explore the visualized paths, view their details, and select the most suitable path for their needs. However, only logged-in users can start recording a trip or contribute new information about the selected paths.
	...
	\item \textbf{Trip Recording and Data Acquisition}:
	      ...
	      A logged-in user starts a trip by choosing a path on the map and selecting the data acquisition mode: manual (distance, time, and average speed only, without automatic detections) or automatic.
	      In automatic mode, BBP uses GPS, an accelerometer, and a gyroscope to track the path and identify potential irregularities in the road surface.
	...
	\item \textbf{Cycling Activity History and Aggregate Statistics}:
	      ...
	      Each trip entry in the list includes summary information such as the date, distance covered, duration, and primary path followed.
	...
	\item \textbf{Trip Specific Statistics and Environmental Data}:
	      ...
	      For each individual trip, BBP displays a map showing the complete path followed by the user and provides total distance, duration, average and maximum speed.
	      At the end of the ride, BBP contacts an external meteorological service to retrieve the weather conditions that were present along the path during the trip.
	      These environmental data, such as average temperature, humidity levels, and wind speed or direction, are stored together with the trip summary and displayed alongside the trip statistics.
	      Additionally, regardless of whether the trip was recorded in automatic or manual mode, the map highlights both the points where the accelerometer detected irregular movements that the user confirmed during the ride and the points where the user submitted manual reports. These markers help identify possible obstacles, rough surfaces, or other issues encountered along the path.
	...
	\item \textbf{Aggregation and Merging of Bike Path Condition Reports}:
	      ...
	      %  TODO add capire che stai biking e proporre trip
