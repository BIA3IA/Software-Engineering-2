...
\begin{itemize}
	\item \textbf{User Registration and Login}:
	      ...
	      Guest Users can browse available bike paths, search for routes between two locations, and receive path suggestions based on community rankings, but they cannot record trips, create new paths, or contribute by making reports.
	...
	\item \textbf{Automatic Trip Interruption on Path Deviation}:
	      ...
	      If the system detects a significant deviation from the planned route, it automatically stops the trip, saves recorded data, and displays a pop-up informing the user of the interruption.
...
\begin{itemize}
	...
	\item \textbf{Logged-in users}:
	      ...
	      If the automatic report mode is enabled, the user can confirm detected obstacles before submission.
	      Furthermore, logged-in users can store their trips within their personal profiles, allowing them to view historical data and general and per-trip statistics such as distance covered, average speed, and total time spent cycling.
\end{itemize}
