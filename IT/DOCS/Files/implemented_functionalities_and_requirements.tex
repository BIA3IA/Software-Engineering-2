...
Authenticated users can also access and modify their personal profile information and application preferences.
They can update their profile information, including name, email address, and password.
Users can also configure application-level preferences, such as the default visibility of newly created paths
and the visual appearance of the app.
...
The app continuously monitors the user’s movement during normal usage to detect potential cycling activity.
Bike activity detection is based on movement speed and consistency over time.

When the app detects that the user is biking without an active trip, it proactively suggests starting a trip.
If the user is currently using the app as a guest, the system prompts them to log in or register in order to 
enable trip monitoring and data persistence.
...
in manual mode, by drawing the route directly on the map, or in automatic mode, by recording a cycling activity
along the desired trajectory. When creating a path, users can choose whether it should remain private
or be shared with the community. Either way, the created path is stored in the system and can be accessed.
...
\subsection{Path Condition Evaluation and Ranking}
\label{subsec:path_condition_evaluation_ranking}

The app evaluates path conditions by combining information collected from user reports.
Reports describe the condition of a specific segment and are used to compute both segment and path status.
Confirmations and rejections are represented as additional reports and contribute to reliability, while report
freshness reduces the influence of older observations. The resulting status is then used to rank paths
returned by the search feature.
...
prioritize recent information. For a report $i$, freshness is computed from its creation time as:
...
Reports may undergo validation by other users and can be confirmed or rejected. Each
confirmation or rejection is stored as a new report on the same segment with status \emph{CONFIRMED} or
\emph{REJECTED}. For each report, two partial scores are computed:
...
Reports in the \emph{CREATED} and \emph{CONFIRMED} states contribute to the confirmed score and those in
\emph{REJECTED} contribute to the rejected score.
...
from report scores weighted by reliability. Rejected reports contribute a negative score to reduce the average.
The path status is then computed by combining segment statuses with a weighted mix:
...
\begin{longtable}{|p{0.07\textwidth}|>{\raggedright\arraybackslash}p{0.65\textwidth}|>{\raggedright\arraybackslash}p{0.23\textwidth}|}
	...
	\textbf{R9}  & The system shall display a pop-up suggesting to start a trip when cycling is detected while no trip is active and the app is open.              & Yes                                     \\ \hline

	\textbf{R10} & The system shall set the current GPS position as trip origin when starting from auto-detection.                                                 & Yes                                     \\ \hline

	\textbf{R11} & The system shall automatically stop the active trip when the user's GPS position deviates from the selected path within a certain threshold.    & Yes                                     \\ \hline

	\textbf{R12} & The system shall allow logged-in users to start a cycling trip in manual or automatic mode.                                                     & Only Manual Mode required \\ \hline
	...
	\textbf{R15} & The system shall collect motion sensor data (accelerometer, gyroscope) during trip recording only when Automatic Mode is enabled.               & Not required                       \\ \hline
	...
	\textbf{R22} & The system shall detect when a user is cycling based on speed and acceleration patterns.                                                        & Yes                                     \\ \hline

	\textbf{R23} & The system shall detect irregular movements from sensor data that may suggest potholes or surface defects when Automatic Mode is enabled.       & Not required                       \\ \hline

	\textbf{R24} & The system shall present automatically detected path and obstacle data to the logged-in user for manual confirmation before publishing.         & Not required                       \\ \hline
	...
	\textbf{R28} & The system shall allow logged-in users to create a new bike path in automatic mode using GPS tracking.                                          & Yes                                     \\ \hline
