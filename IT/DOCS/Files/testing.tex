...
\paragraph{External services (live tests).}
Some integration tests target external services (e.g., real weather, geocoding, and snapping APIs). These tests are intentionally
skipped by default to avoid network instability and long execution time. They can be enabled by setting
the environment variable \texttt{RUN\_LIVE\_TESTS=1} when running the suite.
...
To include live tests that hit external services, run:

\begin{verbatim}
docker compose -f docker-compose.yml -f docker-compose.dev.yml exec -e 
RUN_LIVE_TESTS=1 api npm run test
\end{verbatim}
